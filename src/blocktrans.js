...
exports.parse = function parseBlocktrans(str, line, parser, types, stack, options) {
  ...
  parser.on(types.VAR, function(token) {
    ...
    if(lastKwargName) {
      this.out.push('"' + lastKwargName + '": ' + '_filters.escape(_ctx.' + token.match + ')');
      lastKwargName = null;
      return;
    }
  ...
  parser.on(types.STRING, function(token) {
    if(lastKwargName) {
      this.out.push('"' + lastKwargName + '": _filters.escape(' + token.match + ')');
      lastKwargName = null;
      return;
    }
    return false;
...
exports.compile = function compileBlocktrans(compiler, args, content, parents, options, blockName) {
  ...
  return util.format('_output += _ctx.format(_ctx.gettext("%s"), {' + args.join(',') + '});', str);
};
